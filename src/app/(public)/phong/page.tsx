            ...
            return (
              <div key={rt.id} className="border border-line">
                <div className="grid lg:grid-cols-2">
                  ...
                  >
                    <img
                      src={rt.images[0]?.url || placeholderImage()}
                      alt={rt.name}
                    ...
                    />
                    <span className="absolute left-6 top-6 bg-white/95 px-4 py-2 text-[9px] uppercase tracking-[0.22em] text-ink">
                      <TD>{rt.name}</TD>
                    </span>
                  </div>
                  ...
                  <div className={`p-8 lg:p-12 ${flip ? 'lg:order-1' : ''}`}>
                    <h2 className="text-xl font-light uppercase tracking-[0.18em] text-ink lg:text-2xl">
                      <TD>{rt.name}</TD>
                    </h2>
                    ...
                    <div className="mt-7 flex flex-wrap gap-x-7 gap-y-3 text-[10px] uppercase tracking-[0.16em] text-ink-soft">
                    ...
                    <div className="mt-8 border-t border-line pt-7">
                      ...
                      <div className="mt-4 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                        {amenities.map((a) => (
                          <span
                            key={a}
                            className="flex items-center gap-2 text-xs font-light text-ink-soft"
                          >
                    ...
                    <div className="mt-8 flex flex-wrap items-end justify-between gap-5 border-t border-line pt-7">
                      <div>
                        <span className="text-2xl font-extralight text-ink lg:text-3xl">
                          {formatVND(rt.basePrice)}
                        </span>
                        <span className="ml-1.5 text-[10px] uppercase tracking-[0.18em] text-ink-soft">
                          <T k="rooms.perNight" />
                        </span>
                      </div>
